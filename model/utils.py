        ...
        # After training loops ends, start validation
            ...
            with torch.no_grad():
                ...
                if stop_criteria == 'loss': 
                    ...
                    if test_acc > test_best_acc:  # Compare using test accuracy instead of validation accuracy
                        # Save model
                        torch.save(model.state_dict(), save_file_name)
                        # Track improvement
                        epochs_no_improve = 0
                        ...
                        if epochs_no_improve >= max_epochs_stop:
                            print(
                                f'\nEarly Stopping! Total epochs: {epoch}. Best epoch: {best_epoch} with loss: {valid_loss_min:.2f} and test acc: {100 * test_best_acc:.2f}%'
                            )
                            total_time = timer() - overall_start
                            print(
                                f'{total_time:.2f} total seconds elapsed. {total_time / (epoch+1):.2f} seconds per epoch.'
                            ...
                            # Load the best state dict (based on test acc)
                            model.load_state_dict(torch.load(save_file_name))
                            # Attach the optimizer
                            model.optimizer = optimizer
